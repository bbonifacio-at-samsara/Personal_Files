...
# Grouping by the unique categorical combinations and Expected_Bytes
grouped = df.groupby(['Bitrate', 'Framerate', 'Resolution', 'Bitrate_Control', 'IDR_Interval', 'Codec', 'Stream', 'Expected_Bytes'])
...
for name, group in unique_combinations:
    plt.errorbar(group['Expected_Bytes'], group['mean'], yerr=group['3x_std'], fmt='-o', capsize=5, label=f'{name}')

plt.axhline(y=15, color='r', linestyle='--', label='±15 (Maximum allowed Error)')
plt.axhline(y=-15, color='r', linestyle='--')
plt.title('Mean Bitrate Deviation Percent with 3x Standard Deviation over Different Expected Bytes for Unique Combinations on Local Machine')
plt.xlabel('Expected Bytes')
...
if not os.path.exists(storage):
    os.makedirs(storage)
plt.savefig(storage + 'vs_bitcount_mean_bitrate_deviation_unique_combinations.png', bbox_inches='tight')
plt.show()
